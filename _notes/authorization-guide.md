...
```
⚠️ **Note**: This gives you users/roles/categories but NO transaction data (no demandes, proformas, commandes)
...
```
⚠️ **Note**: `snowly-perms.sql` truncates categories, articles, users, roles, so transaction data from dataV2 may become invalid (foreign key issues)
...
| Département | ID | What they CAN do | What they CANNOT do |
|------------|-----|------------------|---------------------|
...
| vdr_vt_phone | 13 | Téléphonie only |
...
| 13 | vdr_vt_phone | Vendeur | Ventes | 3 | 🔒 Téléphonie + PhonePro only |
...
- Login as `vdr_vt_info` → Try to sell iPhone (Téléphonie) → Should get **error**
...
public void requireLogistiqueDept(Integer idUtilisateur, String action) {
    ...
    if (user.getIdDept() != DEPT_LOGISTIQUE) {  // Add constant: private static final int DEPT_LOGISTIQUE = 3;
        throw new SecurityException("Action '" + action + "' réservée au département Logistique");
    }
}
...
## 🗄️ Database Tables for Permissions
